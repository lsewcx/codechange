 ...
 * @date 2023-7-20
 * @note
 * 实时显示需要有无线而且很卡，建议motion开存图，然后运行image2video合成视频跑debug
...
int main(int argc, char const *argv[]) {
  ...
  while (1) {
    ...
    case RoadType::RingHandle: // 环岛赛道处理 // 环岛赛道处理
      putText(imgaeCorrect, "[2] Ring", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX,
    ...
    putText(imgaeCorrect,
            ...
            Point(COLSIMAGE - 60, 80), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255),
            1); // 车速
