...
void Timer1() {
  ...
  num1++;
  allowRingState = false; // 3秒后取消"圆环状态"
}
...
int main(int argc, char const *argv[]) {
  ...
  while (1) {
    ...
    double gama=motionController.params.gama;
    cv::Mat outputImage = enhanceExposure(frame, gama);
    Mat imageBinary = imagePreprocess.imageBinaryzation(outputImage); // Gray
    ...
    if (motionController.params.CrossEnable) // 赛道元素是否使能
    {
      ...
      if (roadType == RoadType::CrossHandle ||
        ...
        if (crossroadRecognition.crossroadRecognition(
          ...
          // std::thread timer5(Timer2);
          // timer5.detach();
          // std::thread timer6(Timer3);
          // timer6.detach();
        } else
          roadType = RoadType::BaseHandle;
      }
